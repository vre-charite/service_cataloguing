pipeline {
    agent { label 'small' }
    environment {
      imagename_dev = "10.3.7.221:5000/cataloguing"
      imagename_staging = "10.3.7.241:5000/cataloguing"
    ...
    stage('Git clone for dev') {
        ...
        steps{
          script {
          git branch: "k8s-dev",
              url: 'https://git.indocresearch.org/platform/service_cataloguing.git',
              credentialsId: 'lzhao'
            }
        }
    }
      ...
      steps{
        script {
            docker.withRegistry('http://10.3.7.221:5000', registryCredential) {
                customImage = docker.build("10.3.7.221:5000/cataloguing:${env.BUILD_ID}")
                customImage.push()
            }
        }
      }
      ...
      steps{
        script {
            docker.withRegistry('http://10.3.7.241:5000', registryCredential) {
                customImage = docker.build("10.3.7.241:5000/cataloguing:${env.BUILD_ID}")
                customImage.push()
            }
        }
      }
